...
console.log(error)
  ...
  return (
    <div className={`lg:w-[30%] w-full h-full overflow-hidden lg:block bg-slate-200  relative ${!selectedUser?"block":"hidden"}`}>
        <div className='w-[60px] h-[60px] mt-[10px] rounded-full overflow-hidden flex justify-center items-center bg-[#2e6ac9] shadow-gray-500 text-white cursor-pointer shadow-lg fixed bottom-[20px] left-[10px]' onClick={handleLogOut}>
...
{searchData?.map((user)=>(
     ...
     <h1 className='text-gray-800 font-semibold text-[20px]'>{user.name || user.userName}</h1>
      ...
      <div className='w-full h-[300px] bg-[#2e6ac9] rounded-b-[30%] shadow-gray-400 shadow-lg flex flex-col justify-center px-[20px] '>
    <h1 className='text-white font-bold text-[25px]'>VIBE TALK</h1>
